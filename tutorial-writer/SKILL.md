...
- 模式确认后必须通过问答整理“初级需求汇总”，并再次等待用户确认“继续”或修正。
- 只有用户明确回复“继续 / 按这个做 / 你自行判断继续”，才进入正式教程生成流程。
- 除非用户在同一条请求中明确写出“不要弹选项 / 不要问 / 直接生成”，否则不能跳过这两个确认点。
