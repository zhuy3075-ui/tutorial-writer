...
- 只有用户确认自己有复制、复刻或课堂使用权限时，才允许一比一还原参考文档内容和图片；否则只能总结、概括、借鉴结构和排版。
...
- [ ] 若用户提供参考飞书教程链接，已先用可点击选项让用户选择总结借鉴 / 混合复刻 / 一比一还原，并记录选择理由
- [ ] 若选择一比一还原，用户已确认拥有复制、复刻或课堂使用权限；未确认权限时，没有复制大段原文或原图
...
- [ ] 若参考文档图片无法通过 token/url 下载或预览，但用户确认有权限且图片在浏览器可见，已通过截图方式截取图片并插入目标文档同一相对位置；无法截图时已放置图片占位和原因说明
